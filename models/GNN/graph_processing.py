...
def default_categorical_indices(feature_width, kind):
    # TODO: Legacy graph format. Re-enable this block if we switch
    # to graphs generated by ast_vector_parse.py.
    #
    ...
    # TODO: Active default for the new graph format emitted by graph_construction_new.py.
    if kind == "node" and feature_width == 6:
        # New node layout:
...
def build_feature_schema(training_data, testing_data):
    ...
    return FeatureSchema(
        ...
        node_categorical_vocab_sizes=compute_vocab_sizes(
        ...
        edge_categorical_vocab_sizes=compute_vocab_sizes(
...
def apply_normalization_context(samples, context, target_name):
    for sample in samples:
        sample.x = normalize_selected_columns(sample.x, context.feature_schema.node_numeric_indices, context.node_mean, context.node_std)
        sample.edge_attr = normalize_selected_columns(sample.edge_attr, context.feature_schema.edge_numeric_indices, context.edge_mean, context.edge_std)
        sample.recipe = normalize_selected_columns(sample.recipe, context.feature_schema.recipe_numeric_indices, context.recipe_mean, context.recipe_std)
...
def load_graph_file(graph_path):
    """
    Load a serialized PyG graph from a `.pt` file path.
    """
...
def list_graph_design_names(dataset_dir):
    ...
    """
    return {graph_path.stem for graph_path in dataset_dir.glob("*.pt") if graph_path.is_file()}
...
def validate_input_dimensions(training_data, testing_data):
    ...
    def get_recipe_dim(sample):
        ...
        raise TypeError(
            "Unsupported recipe type '{}'. Expected tensor.".format(
