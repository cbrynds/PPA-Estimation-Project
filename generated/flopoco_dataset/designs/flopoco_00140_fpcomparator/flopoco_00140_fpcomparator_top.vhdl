--------------------------------------------------------------------------------
--                     IntComparator_15_111_Freq400_uid4
...
--  approx. output signal timings: XltY: (c0, 0.472000ns)XeqY: (c0, 0.472000ns)XgtY: (c0, 0.472000ns)
...
entity IntComparator_15_111_Freq400_uid4 is
...
architecture arch of IntComparator_15_111_Freq400_uid4 is
...
--------------------------------------------------------------------------------
--                       flopoco_00140_fpcomparator_top
--                      (FPComparator_8_7_Freq400_uid2)
-- VHDL generated for VirtexUltrascalePlus @ 400MHz
-- This operator is part of the Infinite Virtual Library FloPoCoLib
-- All rights reserved 
-- Authors: Florent de Dinechin (2021)
...
--  approx. input signal timings: X: (c0, 0.000000ns)Y: (c0, 0.000000ns)
--  approx. output signal timings: unordered: (c0, 0.000000ns)XltY: (c0, 0.687000ns)XeqY: (c0, 0.687000ns)XgtY: (c0, 0.687000ns)XleY: (c0, 0.687000ns)XgeY: (c0, 0.687000ns)
...
entity flopoco_00140_fpcomparator_top is
    port (clk : in std_logic;
...
architecture arch of flopoco_00140_fpcomparator_top is
   component IntComparator_15_111_Freq400_uid4 is
      port ( clk : in std_logic;
             X : in  std_logic_vector(14 downto 0);
             Y : in  std_logic_vector(14 downto 0);
             XltY : out  std_logic;
...
signal XgtYNum :  std_logic;
   -- timing of XgtYNum: (c0, 0.687000ns)
...
signal XgtYR :  std_logic;
   -- timing of XgtYR: (c0, 0.687000ns)
signal XleYR :  std_logic;
   -- timing of XleYR: (c0, 0.687000ns)
signal XgeYR :  std_logic;
   -- timing of XgeYR: (c0, 0.687000ns)
begin
   excX <= X(17 downto 16);
   excY <= Y(17 downto 16);
   signX <= X(15);
   ...
   positiveY <= not signY ;
   -- expfrac comparisons 
   ExpFracCmp: IntComparator_15_111_Freq400_uid4
      port map ( clk  => clk,
                 X => ExpFracX,
                 Y => ExpFracY,
