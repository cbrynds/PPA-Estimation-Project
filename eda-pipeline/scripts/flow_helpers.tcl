...
proc write_ppa_csv { flow_type } {
  global design platform result_dir
  set csv_path [file join $result_dir "ppa.csv"]
  set write_header [expr {![file exists $csv_path]}]
  set fp [open $csv_path a]
  if { $write_header } {
    puts $fp "design,platform,flow,area_um2,worst_slack_ns,total_negative_slack_ns,clock_period_ns,utilization_pct"
  }
  set area [format %.4f [expr [rsz::design_area] * 1e12]]
  set slack [format %.6f [sta::worst_slack -max]]
  set tns [format %.6f [sta::total_negative_slack -max]]
  set period 0
  if { [llength [all_clocks]] > 0 } {
    set period [get_property [lindex [all_clocks] 0] period]
  }
  set util [format %.2f [expr [rsz::utilization] * 100]]
  puts $fp "$design,$platform,$flow_type,$area,$slack,$tns,$period,$util"
  close $fp
  puts "PPA data written to $csv_path"
}
